...
const sortArray = (array, orderBy, filterBy) => {
    ...
    if (typeof array[0][filterBy] === 'string') {
        ...
        if (orderBy === 'asc') {
            ...
            filteredProducts = [...array].sort((a, b) => a[filterBy].toLocaleLowerCase().localeCompare(b[filterBy].toLocaleLowerCase()));

        } else {

            filteredProducts = [...array].sort((a, b) => b[filterBy].toLocaleLowerCase().localeCompare(a[filterBy].toLocaleLowerCase()));
        }
    ...
    if (typeof array[0][filterBy] === 'object') {
        ...
        if (filterBy === 'category' || filterBy === 'user') {
            if (orderBy === 'asc') {
                ...
                filteredProducts = [...array].sort((a, b) => a[filterBy]?.name.toLocaleLowerCase().localeCompare(b[filterBy]?.name.toLocaleLowerCase()));
            } else {

                filteredProducts = [...array].sort((a, b) => b[filterBy]?.name.toLocaleLowerCase().localeCompare(a[filterBy]?.name.toLocaleLowerCase()));
            }
        }
    ...
    // Filtrado de fechas
    if (filterBy === 'date' || filterBy === 'date_requested' || filterBy === 'date_sended') {
